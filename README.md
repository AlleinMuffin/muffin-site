...
站上所有**版本号、性能数据、已完成事项**都必须能核对，不能凭感觉写。已经踩过一次：

- 启动器写过 `v1.2.0`，实际只有 `alpha-v1.0` / `alpha-v1.1`（以 GitHub Release 为准）
- 性能日志写过「TPS 14 → 19.8」，实际服务器一直稳定在 20 TPS
- 写过「区块预生成」「模组更新检测 v0.3」，实际都没做过

**写法约定**：不确定的事写成「规则 / 计划 / 观察」，不要写成「已完成 / 实测数据」。
需要更正时在文章开头加一段 `> **更正（日期）**：...`，保留透明而不是悄悄改。
